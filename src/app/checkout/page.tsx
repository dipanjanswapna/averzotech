...
const paymentInfoSchema = z.object({
    ...
    expiryDate: z.string().regex(/^(0[1-9]|1[0-2])\/?([0-9]{4}|[0-9]{2})$/, { message: "Invalid expiry date (MM/YY)."}),
...
function DeliveryInfoForm() {
    ...
    return (
        <div className="border rounded-lg p-6">
            <h3 className="text-xl font-semibold mb-4 flex items-center"><MapPin className="mr-2 h-5 w-5"/> Shipping Address</h3>
            <Form {...form}>
...
function ShippingMethodForm() {
     return (
        <div className="border rounded-lg p-6">
            <h3 className="text-xl font-semibold mb-4 flex items-center"><Truck className="mr-2 h-5 w-5"/> Shipping Method</h3>
            <RadioGroup defaultValue="standard" className="space-y-4">
                 <Label className="flex items-center justify-between rounded-lg border p-4 cursor-pointer hover:bg-accent has-[:checked]:bg-accent has-[:checked]:border-primary">
...
function PaymentForm() {
    ...
    return (
         <div className="border rounded-lg p-6">
             <h3 className="text-xl font-semibold mb-4 flex items-center"><CreditCard className="mr-2 h-5 w-5"/> Payment Details</h3>
             <Form {...form}>
                 <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                     <FormField
                         control={form.control}
            ...
            <div className="space-y-4">
                {items.map((item, index) => (
                    <div key={index} className="flex items-center gap-4">
                        ...
                        <p className="font-semibold">BDT {(item.price * item.quantity).toFixed(2)}</p>
                    </div>
                ))}
            </div>
