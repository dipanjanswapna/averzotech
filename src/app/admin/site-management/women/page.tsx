...
export default function WomenPageManager() {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const uploadImage = async (item: any, path: string) => {
        if (!item) return null;
        if (item.file) {
          const storageRef = ref(storage, `site_content/women_page/${path}/${Date.now()}_${item.file.name}`);
      ...
      await setDoc(doc(db, 'site_content', 'women_page'), womenPageContent, { merge: true });

      toast({
        title: 'Success!',
        description: "Women's Fashion page content has been updated.",
      });
    } catch (error: any) {
      console.error(error);
      toast({
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold">Women's Fashion Page Management</h1>
        <p className="text-muted-foreground">
          Manage the content for the public-facing Women's Fashion page.
        </p>
      </div>
      ...
      <div className="flex justify-end sticky bottom-4">
        <Button onClick={handleSaveChanges} disabled={isLoading} size="lg" className="shadow-lg">
          {isLoading ? 'Saving...' : "Save Women's Page Changes"}
        </Button>
      </div>
    </div>
  );
