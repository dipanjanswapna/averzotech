...
export default function HomeLivingPageManager() {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const uploadImage = async (item: any, path: string) => {
        if (!item) return null;
        if (item.file) {
          const storageRef = ref(storage, `site_content/home-living_page/${path}/${Date.now()}_${item.file.name}`);
      ...
      await setDoc(doc(db, 'site_content', 'home-living_page'), homeLivingPageContent, { merge: true });

      toast({
        title: 'Success!',
        description: "Home & Living page content has been updated.",
      });
    } catch (error: any) {
      console.error(error);
      toast({
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold">Home & Living Page Management</h1>
        <p className="text-muted-foreground">
          Manage the content for the public-facing Home & Living page.
        </p>
      </div>
      ...
      <div className="flex justify-end sticky bottom-4">
        <Button onClick={handleSaveChanges} disabled={isLoading} size="lg" className="shadow-lg">
          {isLoading ? 'Saving...' : "Save Home & Living Page Changes"}
        </Button>
      </div>
    </div>
  );
