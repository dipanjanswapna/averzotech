...
export default function KidsPageManager() {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const uploadImage = async (item: any, path: string) => {
        if (!item) return null;
        if (item.file) {
          const storageRef = ref(storage, `site_content/kids_page/${path}/${Date.now()}_${item.file.name}`);
      ...
      await setDoc(doc(db, 'site_content', 'kids_page'), kidsPageContent, { merge: true });

      toast({
        title: 'Success!',
        description: "Kids' Fashion page content has been updated.",
      });
    } catch (error: any) {
      console.error(error);
      toast({
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold">Kids' Fashion Page Management</h1>
        <p className="text-muted-foreground">
          Manage the content for the public-facing Kids' Fashion page.
        </p>
      </div>
      ...
      <div className="flex justify-end sticky bottom-4">
        <Button onClick={handleSaveChanges} disabled={isLoading} size="lg" className="shadow-lg">
          {isLoading ? 'Saving...' : "Save Kids' Page Changes"}
        </Button>
      </div>
    </div>
  );
