...
export default function SportsPageManager() {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const uploadImage = async (item: any, path: string) => {
        if (!item) return null;
        if (item.file) {
          const storageRef = ref(storage, `site_content/sports_page/${path}/${Date.now()}_${item.file.name}`);
      ...
      await setDoc(doc(db, 'site_content', 'sports_page'), sportsPageContent, { merge: true });

      toast({
        title: 'Success!',
        description: "Sports & Activewear page content has been updated.",
      });
    } catch (error: any) {
      console.error(error);
      toast({
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold">Sports & Activewear Page Management</h1>
        <p className="text-muted-foreground">
          Manage the content for the public-facing Sports & Activewear page.
        </p>
      </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {crazyDeals.map((item, index) => (
              <div key={index} className="p-2 border rounded-lg space-y-2">
                ...
                <Input placeholder="Deal Name (e.g. Fitness Trackers)" value={item.name || ''} onChange={(e) => handleTextChange(index, 'name', e.target.value, setCrazyDeals, crazyDeals)} />
                <Input placeholder="Discount Text (e.g. MIN. 30% OFF)" value={item.discount || ''} onChange={(e) => handleTextChange(index, 'discount', e.target.value, setCrazyDeals, crazyDeals)} />
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {shopByCategory.map((item, index) => (
              <div key={index} className="p-2 border rounded-lg space-y-2">
                ...
                <Input placeholder="Category Name (e.g. Cricket)" value={item.name || ''} onChange={(e) => handleTextChange(index, 'name', e.target.value, setShopByCategory, shopByCategory)} />
                <Input placeholder="Discount Text (e.g. UP TO 50% OFF)" value={item.discount || ''} onChange={(e) => handleTextChange(index, 'discount', e.target.value, setShopByCategory, shopByCategory)} />
      ...
      <div className="flex justify-end sticky bottom-4">
        <Button onClick={handleSaveChanges} disabled={isLoading} size="lg" className="shadow-lg">
          {isLoading ? 'Saving...' : "Save Sports Page Changes"}
        </Button>
      </div>
    </div>
  );
