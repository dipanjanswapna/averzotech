...
export default function HomePageManager() {
    ...
    useEffect(() => {
        const fetchHomepageContent = async () => {
            ...
            if (docSnap.exists()) {
                ...
                setHeroImages(data.heroImages.map((img: any) => ({...img,
                    preview: img.url
                })) || []);
            }
        };
    ...
    const handleAddHeroImage = () => {
        setHeroImages([...heroImages, {
            preview: 'https://placehold.co/800x450.png',
            alt: '',
            dataAiHint: ''
        }]);
    };
    ...
    const handleSaveChanges = async () => {
        ...
        try {
            ...
            await setDoc(doc(db, 'site_content', 'homepage'), homepageContent, {
                merge: true
            });
    ...
    return ( <
        ...
        div className = "space-y-4" > {
            heroImages.map((image, index) => ( <
                ...
                className = "flex items-start gap-4 p-4 border rounded-lg bg-secondary/50" >
                <
                ...
                placeholder = "e.g. Fashion sale banner"
                value = {
                    image.alt
                }
                onChange = {
                    (e) => handleHeroTextChange(index, 'alt', e.target.value)
                }
                /> <
                /div> <
                div >
                ...
                onChange = {
                    (e) => handleHeroTextChange(index, 'dataAiHint', e.target.value)
                }
                /> <
                /div> <
                div >
                ...
                className = "text-xs"
                onChange = {
                    (e) => handleHeroImageChange(e, index)
                }
                /> <
                /div> <
                /div> <
